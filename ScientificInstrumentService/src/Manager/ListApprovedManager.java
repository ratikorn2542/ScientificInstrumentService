...
public class ListApprovedManager {

	public List<FlatFeeForm> getlistApproved() {

		ConnectionDB condb = new ConnectionDB();
		Connection con = condb.getConection();
		Statement stmt = null;

		List<FlatFeeForm> listform = new Vector<FlatFeeForm>();
		...
		try {
			...
			String sql = "SELECT sf.sericeRequestFormNumber,sf.type,sf.pickupdate,type_Work_Name,sf.subject,sf.usage_characteristics_Name,sf.period_of_use,sf.sumPrice,sf.additionalcost,sf.booking_status,sf.approveLV"
					...
					+ " where sf.booking_status like 'อนุมัติ'"
					;
			...
			while (rs.next()) {
				...
				Timestamp createdate = rs.getTimestamp(46); 
				List<Rentalschedule> listrentalschedule = this.getlistrentalschedule(sericeRequestFormNumber)  ;
				FlatFeeForm form =  new FlatFeeForm(sericeRequestFormNumber, type, pickupdate, type_Work_Name, subject,
